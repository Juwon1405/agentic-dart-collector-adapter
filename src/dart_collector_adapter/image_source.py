...
Binary resolution order (first hit wins):
    ...
    2. ``DART_VELOCIRAPTOR_BIN`` environment variable
...
def _staged_bin_dir() -> Path:
    # ``.../src/dart_collector_adapter/image_source.py`` -> repo root ``bin/``.
...
def resolve_velociraptor_bin(explicit: str | None = None) -> str:
    ...
    env = os.environ.get("DART_VELOCIRAPTOR_BIN")
    if env:
    ...
    raise VelociraptorNotFoundError(
        ...
        "DART_VELOCIRAPTOR_BIN=/path/to/velociraptor, or pass "
...
def build_remapping_config(image_path: Path, config_path: Path) -> Path:
    ...
    """
    yaml = (
        "# Auto-generated by dart_collector_adapter for dead-disk analysis.\n"
...
def extract_image_to_zip(
    ...
    *,
    ...
    temp_dir = Path(
        tempfile.mkdtemp(prefix="dart-image-", dir=str(temp_root) if temp_root else None)
    )
